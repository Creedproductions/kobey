...
function processYouTubeData(data, url) {
  ...
  // Process ALL formats - don't filter anything
  const allFormats = [];
  ...
  data.items.forEach(item => {
    ...
    const qualityNum = extractQualityNumber(label);
  ...
  const uniqueFormats = deduplicateFormats(allFormats);
  
  // Sort by quality
  uniqueFormats.sort((a, b) => a.qualityNum - b.qualityNum);
  
  console.log(`✅ Total unique formats: ${uniqueFormats.length}`);
  
  // Find default 360p or first available
  const defaultFormat = uniqueFormats.find(f => f.qualityNum === 360) || uniqueFormats[0];
...
function deduplicateFormats(formats) {
  ...
  formats.forEach(format => {
    if (!seen.has(format.qualityNum)) {
      seen.set(format.qualityNum, format);
    }
  });
...
function extractQualityNumber(qualityLabel) {
  ...
  const bitrateMatch = qualityLabel.match(/(\d+)\s*kb\/s/i);
  if (bitrateMatch) return parseInt(bitrateMatch[1]);

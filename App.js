...
// Important for platforms like Koyeb/Render/NGINX proxies
...
function checkCookiesFile() {
  const possiblePaths = [
    process.env.COOKIES_PATH,         // allow override
    '/cookies.txt',
    '/app/cookies.txt',
...
const setupRoutes = () => {
  ...
  app.get('/api/cookies-status', (req, res) => {
    ...
    return res.status(404).json({
      ...
      searchPaths: ['/cookies.txt', '/app/cookies.txt', './cookies.txt']
    });
  });
  ...
  app.get('/health', (req, res) => {
    ...
    return res.status(200).json({
      ...
      features: ['downloads', 'cookies-support', 'merge'],
      database: 'disabled',
  ...
  app.get('/', (req, res) => {
    ...
    return res.status(200).json({
      ...
      note: foundPath
          ? 'Cookies enabled for age-restricted videos'
          : 'Add cookies.txt to root for age-restricted videos'
    });
  });
...
let server; // ✅ global server variable (important for SIGTERM/SIGINT)
...
const startServer = () => {
  // ✅ assign to global "server" (NOT a local const)
  server = app.listen(PORT, () => {
    ...
    if (foundPath) {
      ...
      if (fileInfo.entries) console.log(`📝 Cookie entries: ${fileInfo.entries}`);
    } else {
